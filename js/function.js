$(document).ready(function() {
  (async () => {
    const config = await fetch(CONFIG_PATH).then(res => res.json());

    $(document).find('.logo').html(config.shopName);
      ...
      $.getJSON(`https://gpr-xirdalan.github.io/products.json?v=${Date.now()}`, function(data) {
      ...
      $(document).on('click', '.send-cart', function() {
        ...
        Object.keys(card).map(function(objectKey, index) {
          ...
          $.ajax({
            ...
            data: JSON.stringify({
              newOrder: order,          // передаём как объект, не JSON.stringify
              userConfig: config
            }),
            beforeSend: () => {
      ...
      return `
          <div class="products-card animate__animated animate__fadeIn">
            ...
            <div class="prodcuts-image">
              <img src="https://gpr-xirdalan.github.io/${product.imageSrc}" alt="">
            </div>
